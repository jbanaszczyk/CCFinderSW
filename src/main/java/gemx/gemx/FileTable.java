			...
			pitem.addSelectionListener(new SelectionAdapter() {
				public void widgetSelected(SelectionEvent e) {
					...
					try {
						...
						selected = gemx.utility.ArrayUtil.slice(selections, 0, count);
					} finally {
						if (selected != null) {
...
//					}
	...
	public void updateModel(Model data) {
		Color filteredFileColor = gemx.gemx.scatterplothelper.PlottingColors.getMaskedFileBackground();
...
//				}
	...
	private void searchingId(String text) {
		...
		try {
			final String[] fields = gemx.utility.StringUtil.split(text.substring(1), ',');
